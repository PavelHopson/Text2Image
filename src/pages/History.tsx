...
import { Heart, Download, Trash2, Clock, Filter, X } from 'lucide-react';
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
        ...
        <h2 className="text-xl font-bold text-gray-400 mb-2">История пуста</h2>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setPreview(null)}>
          <div className="relative max-w-4xl w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 glass-card p-4">
              ...
              <p className="text-xs text-gray-500 mt-2 font-mono">{AI_PROVIDERS[preview.provider]?.name} / {preview.model} / {preview.style} / {preview.aspectRatio}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${filter === 'all' ? 'bg-accent/15 text-accent-light' : 'text-gray-400 hover:text-white'}`}
          >
            Все ({items.length})
          </button>
          <button
            onClick={() => setFilter('liked')}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all flex items-center gap-1 ${filter === 'liked' ? 'bg-accent/15 text-accent-light' : 'text-gray-400 hover:text-white'}`}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {filtered.map((img) => (
          <div key={img.id} className="glass-card overflow-hidden group relative animate-slide-up">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => handleLike(img.id)} className={`p-1.5 rounded-lg transition-colors ${img.liked ? 'text-red-400' : 'text-gray-400 hover:text-red-400'}`}>
                    <Heart className={`w-3.5 h-3.5 ${img.liked ? 'fill-current' : ''}`} />
                  </button>
                  <button onClick={() => handleDownload(img)} className="p-1.5 rounded-lg text-gray-400 hover:text-white transition-colors">
                    <Download className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => handleDelete(img.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-400 transition-colors">
